...
const persistShoppingList = function () {
  const shoppingList = Array.from(state.shoppingList);
  localStorage.setItem('shoppingList', JSON.stringify(shoppingList));
};

export const addToShoppingList = function (ingredients) {
  ingredients.map(ing => {
...
export const deleteShoppingListItem = function (ingredient) {
  state.shoppingList.delete(ingredient);
  persistShoppingList();
};
...
const init = function () {
  ...
  if (storage2) state.shoppingList = new Set(JSON.parse(storage2));
};
init();
